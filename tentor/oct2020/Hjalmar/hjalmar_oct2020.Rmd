...
for (i in 1:1000) {
  ...
  dim(cptC) = 2
  dimnames(cptC) = list(c("C1", "C2"))
  
  p_c1 = runif(1) #P(D=2|C=1)
  p_c2 = runif(1) #P(D=2|C=2)
  cptD = matrix(c(1 - p_c1, p_c1, 1 - p_c2, p_c2), nrow = 2, ncol = 2)
  dim(cptD) = c(2, 2)
  dimnames(cptD) = list("D" = c("D1", "D2"), "C" = c("C1", "C2"))
  
  p_c1 = runif(1) #P(A=2|C=1)
  p_c2 = runif(1) #P(A=2|C=2)
  cptA = matrix(c(1 - p_c1, p_c1, 1 - p_c2, p_c2), nrow = 2, ncol = 2)
  dim(cptA) = c(2, 2)
  dimnames(cptA) = list("A" = c("A1", "A2"), "C" = c("C1", "C2"))
  
  p_a1_c1 = runif(1) #P(Y=2|A=1, C=1)
  p_a2_c1 = runif(1) #P(Y=2|A=2, C=1)
  p_a1_c2 = runif(1) #P(Y=2|A=1, C=2)
  p_a2_c2 = runif(1) #P(Y=2|A=2, C=2)
  cptY = matrix(
    c(
      1 - p_a1_c1,
      p_a1_c1,
  ...
  ## Monotone in C
  # P(Y = 2|A = 2, C = 2)
  p1 = querygrain(setEvidence(comfit, c("A", "C"), c("2", "2")), "Y")$Y[2]
  # P(Y = 2|A = 2, C = 1)
  p2 = querygrain(setEvidence(comfit, c("A", "C"), c("2", "1")), "Y")$Y[2]
  # P(Y = 2|A = 1, C = 2)
  p3 = querygrain(setEvidence(comfit, c("A", "C"), c("1", "2")), "Y")$Y[2]
  # P(Y = 2|A = 1, C = 1)
  p4 = querygrain(setEvidence(comfit, c("A", "C"), c("1", "1")), "Y")$Y[2]
  C_monotone = c(C_monotone, isMonotone(p1, p2, p3, p4))

  ## Monotone in D
  # P(Y = 2|A = 2, D = 2)
  p1 = querygrain(setEvidence(comfit, c("A", "D"), c("2", "2")), "Y")$Y[2]
  # P(Y = 2|A = 2, D = 1)
  p2 = querygrain(setEvidence(comfit, c("A", "D"), c("2", "1")), "Y")$Y[2]
  # P(Y = 2|A = 1, D = 2)
  p3 = querygrain(setEvidence(comfit, c("A", "D"), c("1", "2")), "Y")$Y[2]
  # P(Y = 2|A = 1, D = 1)
  p4 = querygrain(setEvidence(comfit, c("A", "D"), c("1", "1")), "Y")$Y[2]
  D_monotone = c(D_monotone, isMonotone(p1, p2, p3, p4))
}
