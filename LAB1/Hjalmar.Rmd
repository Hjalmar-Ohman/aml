...
bn2 = hc(asia, start = NULL, score = "bde", restart = 5, iss = 1000)
arcs(bn2)
vstructs(bn2)
cpdag(bn2)
plot(bn2)
...
predict_mb = function (network, node){
  predictions = c()
  for (i in 1:nrow(test_data)) {
    evidence = setEvidence(network, node, as.vector(as.matrix(subset(test_data, select = node)[i,]))) #????
    query = querygrain(evidence, colnames(test_data)[2])$S
